...
- **Like feedback**: counts stay unchanged while a write is pending, then show the authoritative total separately from confirmation that the visitor's one vote was counted (or already existed). A bounded in-memory cache synchronizes mounted buttons and prevents older snapshots from replacing confirmed totals during the page session. A jump in the total can include other visitors' votes since the static snapshot; it is not attributed to one click.
...
- **Caching**: Next.js build metadata is cached in CI; hashed browser JavaScript uses a one-year immutable cache, preview assets use a seven-day cache with a stable spritesheet-hash query so replacements bypass stale CDN entries, and read-only JSON catalogs use short browser/CDN TTLs. The bundle check rejects missing cache-header rules.
